...
func TestApacheWebServerModule(t *testing.T) {
	...
	apacheT.DefineVerify(func(assert *assert.Assertions) {
		...
		gcloud.Run(t, fmt.Sprintf("compute ssh --zone us-central1-a %s --tunnel-through-iap --project %s", apacheT.GetStringOutput("vm_hostname"), apacheT.GetStringOutput("project_id")))
